...
var missingTransitionTargetErrors = function (definition) {
    // retrieve all states
    var machineStates = [];
    (0, jsonpath_plus_1.JSONPath)({ json: definition, path: '$..States' })
        .forEach(function (s) {
        machineStates = machineStates.concat(Object.keys(s));
    });
    // retrieve all reachable states
    var reachableStates = (0, jsonpath_plus_1.JSONPath)({ json: definition, path: '$..[StartAt,Next,Default]' })
        .filter(function (path, pos, array) { return array.indexOf(path) === pos; });
    // check if all states are reachable
    var unreachable = machineStates.filter(function (state) { return reachableStates.indexOf(state) === -1; })
    ...
    var inexistant = reachableStates.filter(function (state) { return machineStates.indexOf(state) === -1; })
        .map(function (state) { return ({
        'Error code': types_1.StateMachineErrorCode.MissingTransitionTarget,
